...
struct HMap
{
    typedef std::map<uint8_t, uint8_t> mp;
    ...
    HMap(const HMap& copy, std::pair<uint8_t, uint8_t > && swp)
    {
        ...
        if (is_Empty_Column(0)) simetry = true;

#ifndef  NEWT

        if (is_Empty_Column(0))
        {
            std::cout << "Total Iterators: " << iterators + 1 << '\n';
            std::cout << "Min steps(Demo speed boost):" << (history.size()-1) * 2 - 1<<'\n';
            std::cout << "steps\n";

            for (int i = 0; i < history.size()-1; i++)
            {
                //std::cout << "callStack.push([" << int(history[i].first) << ", " << int(history[i].second) << "]);\n";
                std::cout << int(history[i].first) << "->" << int(history[i].second) << '\n';
            }

            for (int i = history.size()-3; i >=0; i--)
            {
                if (history[i].first == 0) history[i].first = 1;
                else
                {
                    if (history[i].first == 1) history[i].first = 0;
                }
                if (history[i].second == 0) history[i].second = 1;      
                else
                {
                    if (history[i].second == 1) history[i].second = 0;
                }
                //std::cout << "callStack.push([" << int(history[i].second) << ", " << int(history[i].first) << "]);\n";
                std::cout << int(history[i].second) << "->" << int(history[i].first) << '\n';
            }
        ...
        if (el == size - 1)
        {
            if (swp.second != 0)
            {
                std::cout << "Total Iterators: " << iterators+1<<std::endl;
                std::cout << "Minimum steps: " << step << std::endl<< "Steps: "<<std::endl;
                for (auto i : history)
                {
    ...
    bool have_empty_column()
    {
        return pool_empty[1] || pool_empty[2] || pool_empty[3];
    }
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    {          
                        ...
                        {       
                            ...
                            {
                                if (!(Recursion[FIRST].have_empty_column() && i != 0 && !Recursion[FIRST].simetry))
                                {
                                    auto data = HMap(Recursion[FIRST], { i,j });

                                    Codes hash = data.Take_Hash();

                                    auto it = Unique.find(hash);
